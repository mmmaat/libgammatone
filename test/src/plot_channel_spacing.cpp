...
int main()
{
  ...
  const auto cf1 = channels::fixed_overlap<T>::center_frequency<B>(fl,fh,step_factor);
  ...
  const auto cf4 = channels::fixed_overlap<T>::center_frequency<B>(fl,fh,0.5);
  const auto cf5 = channels::fixed_size<T>::center_frequency<B>(fl,fh,50);
